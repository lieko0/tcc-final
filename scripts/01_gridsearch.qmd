...
for (i in seq_len(nrow(param_grid))) {
    ...
    iter_time_s <- round((proc.time() - start_time)[[3]], 2)
...
total_gs_time <- round((proc.time() - start_time)[[3]], 2)

gs_timing_summary <- results_df |>
    dplyr::group_by(dataset, neighborhood, topology, rlen, radius, alpha_start, alpha_end) |>
    dplyr::summarise(
        n_folds       = dplyr::n(),
        mean_time_s   = mean(iter_time_s, na.rm = TRUE),
        total_time_s  = sum(iter_time_s, na.rm = TRUE),
        .groups = "drop"
    )

write.csv(gs_timing_summary,
    file.path(OUTPUT_PATH, sprintf("gridsearch_timing_seed%d.csv", SEED)),
    row.names = FALSE
)
cat(sprintf("Tempo total do grid search: %.1fs\n", total_gs_time))
...
sys_info <- data.frame(
    ...
    n_cores_logical = parallel::detectCores(logical = TRUE),
    ram_gb     = tryCatch({
        mem_kb <- as.numeric(gsub("[^0-9]", "",
            readLines("/proc/meminfo", n = 1)))
        round(mem_kb / 1024^2, 1)
    }, error = function(e) NA_real_),
    kohonen_version = as.character(packageVersion("kohonen")),
    total_gs_time_s = total_gs_time
)
write.csv(sys_info,
    file.path(OUTPUT_PATH, "system_info.csv"),
    row.names = FALSE
)
cat("Info do sistema salva.\n")
