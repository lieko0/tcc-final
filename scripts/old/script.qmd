# Script antigo - Configuração e execução dos experimentos
```{r}
#| warning: false
# library("foreign")
# library("tidyverse")
...
TODO
- Gridsearch usando o validation para calcular o QE e comparar os parâmetros
- Gerar um csv com os resultados de QE para cada combinação de parâmetros testada
...
```{r}
current <- "scene-f1-3x3"

tr.labels.matrix <- as.matrix(scene$fold1$tr$labels)
ts.labels.matrix <- as.matrix(scene$fold1$ts$labels)
dplyr::glimpse(tr.labels.matrix)
dplyr::glimpse(ts.labels.matrix)

X.train <- as.matrix(scene$fold1$tr$attr)
X.test <- as.matrix(scene$fold1$ts$attr)
Y.train <- tr.labels.matrix
Y.test <- ts.labels.matrix
...
# Lista das colunas de rótulos
rotulos <- c(
    "Y.Beach", "Y.Sunset", "Y.FallFoliage",
    "Y.Field", "Y.Mountain", "Y.Urban"
)

rotulosPlusCluster <- c(
    "cluster", "Y.Beach", "Y.Sunset", "Y.FallFoliage",
    "Y.Field", "Y.Mountain", "Y.Urban"
)
...
# tr.labels.matrix <- as.matrix(flags$fold3$tr$labels)
# ts.labels.matrix <- as.matrix(flags$fold3$ts$labels)
# dplyr::glimpse(tr.labels.matrix)
# dplyr::glimpse(ts.labels.matrix)

# X.train <- as.matrix(flags$fold3$tr$attr)
# X.test <- as.matrix(flags$fold3$ts$attr)
# Y.train <- tr.labels.matrix
# Y.test <- ts.labels.matrix
...
#     "Y.yellow", "Y.white", "Y.black",
#     "Y.orange"
# )

# rotulosPlusCluster <- c(
#     "cluster", "Y.red", "Y.green", "Y.blue",
#     "Y.yellow", "Y.white", "Y.black",
#     "Y.orange"
# )
...
# dplyr::glimpse(best.result$model$data$Y)
# best.result$model$data$Y
cat(sprintf(
...
# dplyr::glimpse(neuron.dataframes)
```
...
} else if (dir.exists(file.path(current_dir, git_dir))) {
...
while (i < neuron.num) {
    ...
    tryCatch(
        {
            pdf(paste(current, "-", nam, "-", i, ".pdf", sep = ""), width = 11, height = 11)

            plot(best.result$model,
                type = "counts",
                main = paste("SOM - Counts (k =", i, ")")
            )
            ...
            # col 1 - neuron
            # (label.col.left+1) - correção +1 do neuron
            # (label.col.right+2) - correção +1 do neuron & +1 do cluster
            particaoX.info2 <- particaoX.data[, c(1, (label.col.left + 1):(label.col.right + 2))]
            # dplyr::glimpse(particaoX.data)
            # dplyr::glimpse(particaoX.info2)
            ...
            # dplyr::glimpse(totais_rotulos_por_neuronio)
        ...
        error = function(e) {
            cat("Erro ao processar a partição com k =", i, ":", e$message, "\n")
            dev.off()
            gc()
        }
    )
