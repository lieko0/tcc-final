...
# uninstall packages as test



necessary.packages <- c("foreign", "tidyverse", "caret", "kohonen", "dplyr", "tidyr")
installed.packages <- rownames(installed.packages())
i <- 1
for (pkg in necessary.packages) {
    cat (paste("Verificando o pacote", pkg, "(", i, "de", length(necessary.packages), ")...\n"))
    if (!pkg %in% installed.packages) {
        cat(paste("Por favor instale o pacote", pkg, "...\n\n  install.packages(\"",pkg,"\", dependencies=TRUE)\n\n"))
        
    } else {
        cat(paste("O pacote", pkg, "já está instalado.\n\n"))
    }
    i <- i + 1
}
```
